...
namespace StorageSystem.WEB.Pages.MeasurementUnits
{
    public partial class MeasurementUnitsIndex
    {
        ...
        private async Task<bool> LoadListAsync(int page)
        {
            ...
            var url = $"api/MeasurementUnits/?page={page}&recordsnumber={RecordsNumber}";
            if (!string.IsNullOrWhiteSpace(Filter))
            {
                url += $"&filter={Filter}";
            }
            var responseHttp = await Repository.GetAsync<List<MeasurementUnit>>(url);
        ...
        {
            ValidateRecordsNumber(RecordsNumber);
            var url = $"api/MeasurementUnits/totalPages?recordsnumber={RecordsNumber}";
            if (!string.IsNullOrWhiteSpace(Filter))
            {
                url += $"&filter={Filter}";
            }
        ...
        private async Task DeleteAsync(MeasurementUnit measurementUnit)
        {
            ...
            {
                Title = "Confirmacion",
                Text = $"¿Estas seguro que quieres borrar la unidad de medida: {measurementUnit.Name}?",
